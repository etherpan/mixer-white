<template>
  <b-tab-item :label="$t('deposit')" header-class="button_tab_deposit xee-tlbr">
    <fieldset>
      <b-field :label="$t('token')" data-test="token_list_dropdown">
        <b-dropdown v-model="selectedToken" expanded aria-role="list">
          <div slot="trigger" class="control">
            <div class="input text-black-xee">
      ...
      <p class="text-center fw-bold text-black-xee mt-7">Select Amount</p>
      <b-field data-test="token_list_dropdown" class="mb-5 mt-3">
        <b-dropdown v-model="currentStep" expanded aria-role="list" @change="changeAmount">
          <div slot="trigger" class="control">
            <div class="input text-black-xee">
              <span>{{ shortenAmount(amounts[currentStep].amount) }}</span>
            </div>
          </div>
          <b-dropdown-item v-for="({ amount }, key) in amounts" :key="key" aria-role="listitem" :value="key">
    ...
    <connect-button
      v-if="!isLoggedIn"
      outlined
      type="is-primary is-fullwidth deposit-button-xee"
      data-test="button_connect"
      action-text="Connect Wallet To Deposit"
    />
    <b-button
      v-else
      outlined
  ...
  methods: {
    ...
    openDepositModal() {
      this.$store.dispatch('application/prepareDeposit', {
        prefix: `white-${this.selectedToken}-${this.selectedAmount}-${this.netId}`
      })
