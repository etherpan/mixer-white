...
class EventService {
  ...
  async updateEvents(type, cachedEvents) {
    ...
    console.log('log->newEvents', newEvents, fromBlock)
  ...
  async getBlocksDiff({ fromBlock }) {
    ...
    console.log('log->currentBlockNumber', currentBlockNumber)
    console.log('log->fromBlock', fromBlock)
    console.log('log->difference', Math.ceil(currentBlockNumber - fromBlock))
    return {
      currentBlockNumber,
  ...
  async getBatchEventsFromRpc({ fromBlock, type }) {
    try {
      const blockRange = 4950
      const { blockDifference, currentBlockNumber } = await this.getBlocksDiff({ fromBlock })
      console.log('log->blockDifference', blockDifference, currentBlockNumber)
      let numberParts = blockDifference === 0 ? 1 : Math.ceil(blockDifference / blockRange)
      const part = Math.ceil(blockDifference / numberParts)
  ...
  async getEventsFromRpc({ fromBlock, type }) {
    try {
      ...
      if (Number(this.netId) === 56 || Number(this.netId) === 97) {
  ...
  async getEventsFromBlock({ fromBlock, graphMethod, type }) {
    try {
      ...
      console.log('log->allEvents', allEvents)
      if (allEvents.length) {
        return {
          events: allEvents,
