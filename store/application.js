...
const getters = {
  ...
  tornadoProxyContract: (state, getters, rootState) => ({ netId }) => {
    const {
      ...
      'white-proxy-light.contract.tornadocash.eth': tornadoProxyLight
  ...
  hasEnabledLightProxy: (state, getters, rootState, rootGetters) => {
    return Boolean(rootGetters['metamask/networkConfig']['white-proxy-light.contract.tornadocash.eth'])
  },
...
const actions = {
  ...
  async createSnarkProof(
    ...
    { fee, root, note, tree, recipient, netId, leafIndex }
  ) {
    ...
    if (netId === '56' || netId === '97') {
      relayer = BigInt('0x978A1d171DD13aB4c6fbf6C50A4884f4d926596C')
    } else {
      relayer = BigInt('0x978A1d171DD13aB4c6fbf6C50A4884f4d926596C')
    }
  ...
  async prepareWithdraw({ dispatch, rootState, getters, commit }, { note, recipient }) {
    ...
    try {
      ...
      const { ethAccount } = rootState.metamask
      console.log('log->ethAccount', ethAccount)
      const netId = 56
      const contractInstance = getters.tornadoProxyContract({ netId })
      ...
      const fee = BigInt(parsedNote.amount * 10 ** 16)
