...
export const actions = {
  ...
  async relayTornadoWithdraw({ state, commit, dispatch, rootState }, { note }) {
    ...
    // const { currency, netId, amount } = parseNote(note)
    ...
    try {
      ...
      if (netId === '56' || netId === '97') {
        apilink = 'https://bsc-relayer.herokuapp.com/relay'
      }
      ...
      if (response.status === 200) {
        ...
        commit('SAVE_JOB', {
          ...
          type: 'white',
          action: 'Deposit',
          relayerUrl: 'eth-relayer.herokuapp.com',
          commitmentHex,
          amount,
          currency,
          timestamp,
        ...
        dispatch('runJobWatcherWithNotifications', { id, type: 'white', netId })
      } else {
    ...
    return new Promise((resolve, reject) => {
      const getConfirmations = async ({ id, netId, type, noticeId, retryAttempt = 0, noticeCalls = 0 }) => {
        try {
          ...
          if (netId === '56' || netId === '97') {
            apilink = `https://bsc-relayer.herokuapp.com/v1/jobs/${id}`
          }
          const response = await fetch(apilink, {
            method: 'GET',
            mode: 'cors',
  ...
  async handleResponse(
    ...
    if (txHash && noticeCalls === 0 && (Number(confirmations) > 1 || status === 'CONFIRMED')) {
      noticeCalls++
